...
exports.addTwigExtensions = function ( Twig ) {
  var twigExtensions = require( path.resolve( 'lib/twig' ) );

  [ 'filters', 'functions', 'tags' ].forEach(function ( type ) {
    for ( var name in twigExtensions[ type ] ) {
  ...
  return new Promise(function ( fulfill, reject ) {
    gulp.src( 'data/**/*.json' )
      ...
      .on('data', function ( file ) {
        var fileObj = {};
        var stem = path.basename( file.path, path.extname( file.path ) );
...
function loadSitemap( context ) {
  ...
  return new Promise(function ( fulfill, reject ) {
    gulp.src( 'src/content/**/*.html' )
      .pipe( gulpSort() )
      .on( 'data', function ( file ) {
        var page = fm( String( file.contents ) ).attributes;
...
function pathObj( objPath, obj ) {
  ...
  return dirs
    .split( path.sep )
    .reduceRight(function( prev, curr, i, arr ) {
      var next = {};
      next[ curr ] = prev;
      return next;
    }, obj );
